...
lattice =np.loadtxt('pca_gauss_lattice.txt')

plt.pcolormesh(np.arange(0, 2, 0.1), np.arange(0, 2, 0.1), lattice.T, cmap='RdBu')
plt.xlabel("Training dataset max blur")
plt.ylabel("Testing dataset max blur")
plt.title("PCA model with Gaussian Blur")
plt.show()
